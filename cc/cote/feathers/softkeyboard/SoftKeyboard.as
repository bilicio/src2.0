...
package cc.cote.feathers.softkeyboard
{
	...
	public class SoftKeyboard extends FeathersControl
	{
		...
		public function SoftKeyboard(
			...
			this.width = 320 * multiplier;
			this.height = 160 * multiplier;
		}
...
//				throw(error);
		...
		private function _onKeyDown(e:KeyEvent):void {
			...
			// Re-dispatch the event so implementors can easily listen on the SoftKeyboard object --------------------------- SABER O NUMERO DA TECLA PRESSIONADA
			// directly
		//	trace('trace: ' + e);
			dispatchEvent(e);
		...
		public function setKEY(e, textField:TextFieldTextRenderer, allTextFields:Array, options:Array):String{
			/*try{
				trace(e.char);
				
				if(textField.text == 'NOME'){
					textField.text = e.char.toUpperCase();
				}else{
					textField.text += e.char.toUpperCase();
				}
			}catch(err){};*/
			//TweenMax.to(e.target, .1, {scaleX:.9, scaleY:.9, yoyo:true, repeat:1, ease:Quad.easeOut});
			
			
			var texto:String = "";
			if(e.char != null){
				for(var i:Number=0; i<allTextFields.length; i++){
					/*switch(textField){
						case allTextFields[i][1]:
					...
					if(textField == allTextFields[i][1]){
						
						if(textField == allTextFields[i][1]){
							if(textField.text.length >= Number(options[i][1])){
								return textField.text;
							}
						}
						
						if(textField.text == allTextFields[i][0]){
							
								//trace(options[i][0] + " - " + Number(e.char));
							if(options[i][0] == 'numbers'){
								if(!isNaN(Number(e.char))){
									texto = e.char
								}
							}else if (options[i][0] == 'letters'){
								if(isNaN(Number(e.char))){
									texto = e.char
								}
							}else{
							...
							if(options[i][0] == 'numbers'){
								if(!isNaN(Number(e.char))){
									if(allTextFields[i][0] == 'TELEFONE'){
										texto = textField.text + e.char;
										if(texto.length >= 2 && texto.indexOf('(') == -1){
											texto = '('+texto.substr(0,2)+')' + texto.substr(2,10);
										}else{
											//texto = textField.text + e.char;
										}
									}else{
				...
				if(e.charCode != 8){
					for(var a:Number=0; a<allTextFields.length; a++){
						if(textField == allTextFields[a][1]){
							if(textField.text.length >= Number(options[a][1])){
								return textField.text;
							}
						}
					}
				...
				switch(e.charCode){
					case 8 : 
						texto = textField.text.substr(0,textField.text.length-1);
						
						for(var d:Number=0; d<allTextFields.length; d++){
							if(textField == allTextFields[d][1]){
								if(allTextFields[d][0] == 'TELEFONE'){
									if(texto.length <= 4){
										trace('é 4');
										texto = textField.text.substr(1,2);
									}
								}
							}
						}
					...
					case 0000000 : 
						for(var c:Number=0; c<allTextFields.length; c++){
							if(textField == allTextFields[c][1]){
								trace('find' + options[c][0]);
								if(options[c][0] == undefined){
									texto = textField.text + ".com";
									//trace(e.charCode);
								}else{
									texto = textField.text;
								}
					...
					case 111111 : 
						for(var b:Number=0; b<allTextFields.length; b++){
							if(textField == allTextFields[b][1]){
								if(options[b][0] == undefined){
									texto = textField.text + ".br";
									//trace(e.charCode);
								}else{
									texto = textField.text;
								}
		...
		override public function dispose():void{
			...
			for (var i:uint = 0; i < _configurations[_currentLayoutIndex].numChildren; i++) {
				
				var kr:KeyRow = _configurations[_currentLayoutIndex].getChildAt(i) as KeyRow;
