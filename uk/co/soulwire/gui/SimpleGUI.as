...
package uk.co.soulwire.gui
{
	...
	public class SimpleGUI extends Sprite
	{
		...
		private var _leftPos:Boolean = false;
		...
		public function SimpleGUI(target : DisplayObjectContainer, title : String = null, hotKey : * = null, leftPos = false)
		{
			_title = title;
			_target = target;
		...
		public function save() : String
		{
			...
			var output : String = _title != null?_title+'\n':'';
			...
			for (var i : int = 0; i < _components.length; i++)
			{
				...
				if (options.hasOwnProperty("target"))
				{
					...
					for (var j : int = 0; j < targets.length; ++j)
					{
						...
						//trace('OUTPUT: ' + path + ' - ' + target[prop] + ' - ' + prop + ' - ' + target);
						
						/*try
						{
							
							trace('OUTPUT TRY: ' + (component as FileChooser).file.name);
							
						} 
						catch(err:Error){}*/
						
						if(component is FileChooser){
							output += path + " = " + (component as FileChooser).file.name + ';\n';
						}else{
		...
		public function addControl(type : Class, options : Object, texto = '') : Component
		{
			...
			for (var option : String in options)
			{
				if (component.hasOwnProperty(option))
				{
					...
					//trace('option: ' + component + ' | ' + options[option]);
					//trace('set: ' + component[option] + ' - ' + options[option]);
					if (component is CheckBox){
						
						//(component as CheckBox).selected = false;
						
						if(options[option] == 'true'){
							//trace('set: ' + options[option]);
							(component as CheckBox).selected = true;
						}
					...
					if (component is InputText){
						(component as InputText).text = options[option];
					}
					
					
					try{
						//trace(component['unify']);
					}catch(err){};
				}
			}
			...
			if (!component.hasOwnProperty("label") && options.hasOwnProperty("label") && type !== Label)
			{
				...
				/*if(type == InputText){
					(component as InputText).text = texto;
				}*/
		...
		public function addInput(text : String, options : Object = null) : void
		{
			...
			addControl(InputText, merge(params, options), options.text);
		}
		...
		private function initToolbar() : void
		{
			...
			_version = new Label();
			_version.text = "Antidoto Config v" + VERSION;
			_version.alpha = 0.5;
			...
			for (i = 0; i < _components.length; i++)
			{
				...
				if (options.hasOwnProperty("target"))
				{
					...
					for (j = 0; j < targets.length; j++)
					{
						...
						if (component is CheckBox)
						{
						//	trace('unify: ' + component["unify"])
							if(component["unify"] == false){
		...
		private function draw() : void
		{
			...
			for (i = 0; i < _container.numChildren; i++)
			{
				...
				for (j = 0; j < column.numChildren; j++)
				{
					...
					for (k = 0; k < group.numChildren - 1; k++)
					{
						...
						bounds = component.getBounds(component);
						//bounds.bottom += COMPONENT_MARGIN / 2; //----------------------------------------------------------------- TIREI
			...
			_toolbar.graphics.drawRect(0, 0, _width - _toolbar.x + 50, TOOLBAR_HEIGHT);
		...
		private function onAddedToStage(event : Event) : void
		{
			...
			if(_leftPos){
				_toggle.x = _stage.stageWidth - MARGIN - TOOLBAR_HEIGHT;
				_toolbar.x = _stage.stageWidth - MARGIN - TOOLBAR_HEIGHT;
				_container.x = _stage.stageWidth - MARGIN - TOOLBAR_HEIGHT - 200;
			}else{
				_toggle.x = MARGIN;
				_toolbar.x = MARGIN;
				_container.x = MARGIN;
		...
		{
			_target.removeEventListener(Event.ADDED, onTargetAdded);
			if (!_hidden) show();
		}
		...
		public function dispose():void{
			...
			_container.removeEventListener(Event.ENTER_FRAME, onEnterFrame);
			_toggle.removeEventListener(MouseEvent.CLICK, onToggleClicked);
			
			var column:Sprite;
			var group:Sprite;
			
			for (var i:Number = _container.numChildren; i--;)
			{
				column = _container.getChildAt(i) as Sprite;

				for (var j:Number = column.numChildren; j--;)
				{
					group = column.getChildAt(j) as Sprite;

					for (var k:Number = group.numChildren; k--;)
					{
						 group.graphics.clear();
						 group.removeChildAt(k);
					}
...
internal class FileChooser extends Component
{
	...
	protected var _value:String = 'ok';
